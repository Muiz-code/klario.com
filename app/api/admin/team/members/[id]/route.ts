...
import { updateMember, deleteMember, listRoles, getMemberById } from "@/lib/db/rbac";
import { deleteAuthUser } from "@/lib/auth/authUsers";
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Remove the Supabase auth account too, so the email can be re-invited fresh.
  if (member?.email) await deleteAuthUser(member.email);
  await logMemberAction(access.email, "remove_member", member?.email ?? id);
